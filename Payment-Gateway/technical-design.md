...
- **Order status:** `null`, `Pending`, `Paid`, `PaymentFailed`, `canceled`, `Error`, `Refunded`, `Committed`, `Declined`
- **Payment status:** `null`, `Due`, `AuthChallenged`, `Auth`, `Failed`, `canceled`, `Error`, `Void`, `Refund`, `PaymentDeclined`
